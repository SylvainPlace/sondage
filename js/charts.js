...
export function updateAnecdotes(data, show = true) {
  ...
  if (!show) {
    list.innerHTML =
      '<p style="color:var(--text-muted)">Veuillez choisir un filtre pour voir les commentaires.</p>';
    return;
  }
  ...
  if (withConseil.length === 0) {
    list.innerHTML =
      '<p style="color:var(--text-muted)">Aucun commentaire pour cette sélection.</p>';
    return;
  }

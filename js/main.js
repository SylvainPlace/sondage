...
function updateStats() {
  ...
  if (salairesTotaux.length > 0) {
    ...
    if (salairesTotaux.length % 2 !== 0) {
      ...
      medianTotal = Math.round(
        (salairesTotaux[midTotal - 1] + salairesTotaux[midTotal]) / 2
      );
    }
  }
  ...
  document.getElementById("avg-salary").textContent =
    salairesNumeriques.length > 0 ? formatMoney(mean) : "- €";
  document.getElementById("median-salary").textContent =
    salairesNumeriques.length > 0 ? formatMoney(median) : "- €";
  document.getElementById("avg-salary-total").textContent =
    salairesTotaux.length > 0 ? formatMoney(meanTotal) : "- €";
  document.getElementById("median-salary-total").textContent =
    salairesTotaux.length > 0 ? formatMoney(medianTotal) : "- €";
